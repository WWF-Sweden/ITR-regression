...
# All filters are removed, except for the latest_peak filter which as been updated to only include model-scenarios that have a peak year of emissions after 2023 (excluding model-scenarios with peak year 2015 and 2020) 
...
latest_peak <- c(2025, 2030, 2035, 2040, 2045, 2050, 2055, 2060, 2065, 2070, 2075, 2080, 2085, 2090, 2095, 2100)
  # Excluding Model-scenarios with peak year 2015 and 2020 since emissions did not peak in 2015 or 2020, but only increased since then (from a global perspective) -  https://ourworldindata.org/co2-and-greenhouse-gas-emissions

# Number of combinations of filters
nfilters <- 1  # Update this value if adding more filters. In this code, we currently only have one filter, latest_peak above. 

# Filter the data to include only the Model-Scenarios with a peak year in latest_peak
filtered_dfs <- AR6_prefilter %>% filter(Year_of_peak_GHG_Emissions %in% latest_peak)
...
if(write_csvs) {
  write_excel_csv(filtered_dfs, path=paste0('ITR-reg-scenariodata_csv_sector_2/Scenario_dataset_detailed-', nfilters, '.csv'))
}
...
if(write_csvs) {
  write.xlsx(summary, 'ITR-reg-scenariodata_csv_sector_2/Scenario_dataset_summary.xlsx', overwrite = TRUE)
}
