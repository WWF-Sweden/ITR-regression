...
# Currently, the ETP data is not used. 
...
# # Included more variables in df for AR6 compared to SR1.5
em0 <- 'Emissions|CO2'
# Below is the list of variables included in the current WWE/CDP methodology. In this version of the R-code, the number of variables included increased to add additional more sector-specific variables. See below. 
...
# Adding additional sector-specific CO2 emissions mitigation variables
em_subs <- c('AFOLU', 'Energy', 'Energy and Industrial Processes', 'Energy|Demand', 'Energy|Demand|Industry', 'Energy|Demand|Industry|Cement','Energy|Demand|Industry|Chemicals',
  ...
  # Based on variables in the underlying data. Note that more variables are added.  
...
# NOTE THAT THIS PART OF THE R-CODE IS SIGNIFICANTLY UPDATED COMPARED TO THE SR1.5 METHOD. 
# In this version, a df with scenarios that delay action, i.e., scenarios no late peak year, rely on too much cdr to achieve the temperature, and scenarios within the category C1 and C2 (1.5 scenarios). 
...
# Print AR6_prefilter for analysis purpose
write.csv(AR6_prefilter, file = "C:/Users/amanda.kihlman/R-test/AR6_Implementering/Input/ITR-reg-scenariodata_csv_sector_nofunkyscenarios/AR6_prefilter.csv", row.names = FALSE)

# Exclude model-scenarios with peak year 2015 and 2020
exclude_peak_years <- c(2015, 2020)

# Create a dataframe with Model-Scenarios that have a Year_of_peak_GHG_Emissions of 2015 or 2020
exclude_peak_years_df <- AR6_prefilter %>% filter(`Year_of_peak_GHG_Emissions` %in% exclude_peak_years)

# Define the criteria for scenarios that delay action, i.e., funky scenarios
categories <- c("C1", "C2", "C3", "C4")
#latest_peak <- c(2070, 2075, 2080, 2085, 2090, 2095)
cdr_max <- 10000

# Create a new df including only the funky scenarios
Funkyscenarios_criteria5 <- AR6_prefilter %>% filter(Category %in% categories,`cdr|max` > cdr_max)

# Start with AR6_prefilter and apply all filters
filtered_dfs <- AR6_prefilter %>% 
  filter(!(`Model-Scenario` %in% Funkyscenarios_criteria5$`Model-Scenario`)) %>%
  filter(!(`Model-Scenario` %in% exclude_peak_years_df$`Model-Scenario`))


# Number of combinations of filters
nfilters <- 1  # Update this value if adding more filters.


# Calculate the nscenariosv column
nscenariosv <- length(unique(filtered_dfs$`Model-Scenario`))

# Print the number of unique scenarios
print(nscenariosv)

# Add the V8 column
filtered_dfs <- filtered_dfs %>% mutate(V8 = row_number())

# Write the filtered data to a csv file
if(write_csvs) {
  write_excel_csv(filtered_dfs, path=paste0('ITR-reg-scenariodata_csv_sector_nofunkyscenarios/Scenario_dataset_detailed-', nfilters, '.csv'))
}

# Create a summary of the data including number of rows and unique scenarios, given the exclusion of model-scenarios with a latest peak in 2015 and 2020:
summary <- data.frame(V8 = 1:nfilters, nscenariosv = nscenariosv)


# If you want to write the summary to an Excel file
if(write_csvs) {
  write.xlsx(summary, 'ITR-reg-scenariodata_csv_sector_nofunkyscenarios/Scenario_dataset_summary.xlsx', overwrite = TRUE)
}

### Print additional df for analysis #### 

# Print exclude_peak_years_df for analysis purpose
write.csv(exclude_peak_years_df, file = "C:/Users/amanda.kihlman/R-test/AR6_Implementering/Input/ITR-reg-scenariodata_csv_sector_nofunkyscenarios/exclude_peak_years_df.csv", row.names = FALSE)

# Print funkyscenarios for analysis purpose
write.csv(Funkyscenarios_criteria5, file = "C:/Users/amanda.kihlman/R-test/AR6_Implementering/Input/ITR-reg-scenariodata_csv_sector_nofunkyscenarios/funkyscenarios_Criteria5.csv", row.names = FALSE)
