...
# Section 1: defines path to AR6 data and ETP data, which could be used for a couple intensity denominators (e.g., steel production)
...
# doi: 10.5281/zenodo.5886911 | url: data.ece.iiasa.ac.at/ar6/ DOI
  ...
  # IEA ETP 2017 data is optionally used to test physical intensity benchmarks in combination with AR6. These data can be acquired from the IEA, but are not necessary to run either of the R scripts included in the repository. Note that this data is currently not being used, the code are only prepared for using such data. 
...
# Instead, these signs are replaced by underscore (_) in the names of the variables.  
...
# By default, ETP data is not used; but if a data is provided and use_etp is set to TRUE, ETP data is combined with AR6 data to calculate a few intensity indicators.
...
# Read Meta data from file AR6_Scenarios_Database_metadata_indicators_v1.1, from the sheet meta_Ch3vetted_withclimate, as this sheet contains only scenarios that undergone through IPCC's vetting process. For more information regarding the vetting process, please follow this link: https://www.ipcc.ch/report/ar6/wg3/downloads/report/IPCC_AR6_WGIII_Annex-III.pdf
...
calculate.AFOLU.cs <- function(df) {
  # Add a column estimating land use-related carbon sequestration due to poor reporting of 'Carbon Sequestration|Land Use' in AR6 data. This was similar in the SR15 database. 
...
# Some of the calculations are based on the ETP file, loaded in section 1. If this code is TRUE: use_etp <- TRUE (in section 1), then the code block below will be executed. This is however set to FALSE meaning that the ETP file is note used. 
calculate.intensity.vars <- function(df, use_etp) {

# The intensity figures calculated below are based on the underlying data and variables in AR6_Scenarios_Database_World_v1.1 
...
# The intensity figures are based on the ETP data. Note that this is note used since the file is not used. However, if wish to be used, the code below prepare for it - however, make sure to update the file a newer file, the current one is from 2017.  
  if(use_etp) {
...
# This is currently not used in the methodoloy, however, the code is kept for now
...
# Included more variables in df for AR6 compared to SR15.
em0 <- 'Emissions|CO2'

# Adding additional sector-specific CO2 emissions mitigation variables. Note that all of them is note used in the linear regression output, however, they are kept here if wishes to be used. 
...
#Below is only for test/visulazation purposes:
print(em0)
print(em_subs)
...
# Create a summary of the data including number of rows and unique scenarios:
