...
# Section 2. Load data output files generated in the first part of the R-code (Preparation of data) and xlsx created based on the underlying input AR data.   
...
# Selecting target benchmarks (mitigation variables). Note that not all variables are used in the tool, but included for test reasons. 
...
# Suggest excluding the below code before we publish the script ince we are not using it. 
